...
// =============================================================================
// THE A 5995 - Admin Projects List Page
// =============================================================================

import { useState, useEffect, useCallback } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { getLocalizedField, formatDate, cn } from '@/lib/utils';
import {
  Plus,
  Search,
  Edit,
...
import type { ProjectWithDetails, PaginatedResponse } from '@/types';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-heading font-bold text-primary-700">Projects</h1>
          <p className="text-luxury-500 mt-1">{total} projects total</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-luxury-200 overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center">
            ...
            <p className="text-luxury-500 text-sm">Loading projects...</p>
          </div>
        ) : projects.length === 0 ? (
          <div className="p-12 text-center">
            <Building2 className="w-12 h-12 mx-auto text-luxury-300 mb-3" />
            <p className="text-luxury-500">No projects found</p>
            <Link
              href="/admin/projects/new"
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-luxury-100">
                {projects.map((project) => (
                  <tr key={project.id} className="hover:bg-luxury-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="w-12 h-12 bg-luxury-100 rounded-lg overflow-hidden flex-shrink-0">
                        {project.images?.[0]?.url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img
                            src={project.images.find((i) => i.is_primary)?.url || project.images[0].url}
                            alt={project.name_en}
                    ...
                    <td className="px-4 py-3 text-xs text-luxury-500">
                      {formatDate(project.created_at, 'en')}
                    </td>
                    <td className="px-4 py-3">
